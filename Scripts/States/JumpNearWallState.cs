...
public class JumpNearWallState : BaseState
{
  ...
  public JumpNearWallState(string name, Player player, HitConsumer hitConsumer, IgnoreSurroundings ignoreSurroundings) : base(name)
  {
    _player = player;
  ...
  {
    base.PhysicsUpdate();
    //* move inversed wall allowed only
